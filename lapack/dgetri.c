/* dgetri.f -- translated by f2c (version 19970805).
...
/*     and the inverse is not computed. */
    ...
    dtrtri_("Upper", "Non-unit", n, &a[a_offset], lda, info, 5L, 8L);
    if (*info > 0) {
	return 0;
    }
...
/*           Compute current block column of inv(A). */
		...
		dgemm_("No transpose", "No transpose", n, &jb, &i__2, &c_b20, 
			...
			ldwork, &c_b22, &a[j * a_dim1 + 1], lda, 12L, 12L);
	    }
